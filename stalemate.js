...
const canMoveTo = (Piece, rowMoveTo, columnMoveTo) => {
  if (!cells[rowMoveTo][columnMoveTo]) {
    ...
    if (enemyPiece.color === vars.oppositeColor) {
      if (!checkAfterMove(Piece, rowMoveTo, columnMoveTo, enemyPiece))
        return true;
    }
  }
};
...
const moveExist = {
  ...
  Pawn: (piece, column, row) => {
    if (
      !cells[row - 1][column] &&
      !checkAfterMove(piece, row - 1, column, null)
    )
      return true;
    let enemyPiece;
    if (column > 0) {
      ...
      if (CellsMinus1Element) {
        ...
        if (
          ...
          !checkAfterMove(piece, row - 1, column - 1, enemyPiece)
        )
          return true;
      }
    }
...
const stalemate = () => {
  for (const Piece of pieces) {
    if (
      ...
      moveExist[Piece.type](Piece, Piece.column, Piece.row)
    )
      return;
  }
  return "stalemate";
