...
const attack = (
  ...
  for (i; i <= countOfPieces; i++) {
    ...
    if (currentPiece.canMove(attackForRow, attackForColumn, moveType))
      return currentPiece;
  }
};
...
const moves = {
  Pawn: (Piece, toRow, toColumn, opponentPiece) => {
    if (Piece.row !== 1) {
      ...
      if (
        (toColumn - Piece.column === 1 || toColumn - Piece.column === -1) &&
        opponentPiece
      ) {
        if (!checkAfterMove(Piece, toRow, toColumn, opponentPiece)) {
          pieces[cells[0][toColumn]].HTMLImage.style.backgroundColor =
            "#d5cd7f";
          const finishImages = document.getElementsByClassName(
            `${vars.color}FinishImages`,
          );
  ...
  Knight: (Piece, toRow, toColumn, opponentPiece) => {
    if (
      canPieceMove["Knight"](Piece.row, Piece.column, toRow, toColumn) &&
      !checkAfterMove(Piece, toRow, toColumn, opponentPiece)
    ) {
      doMove(Piece, toRow, toColumn, opponentPiece);
      return true;
  ...
  Bishop: (Piece, toRow, toColumn, opponentPiece) => {
    if (
      canPieceMove["Bishop"](Piece.row, Piece.column, toRow, toColumn) &&
      !checkAfterMove(Piece, toRow, toColumn, opponentPiece)
    ) {
      doMove(Piece, toRow, toColumn, opponentPiece);
      return true;
  ...
  Rook: (Piece, toRow, toColumn, opponentPiece) => {
    if (
      canPieceMove.Rook(Piece.row, Piece.column, toRow, toColumn) &&
      !checkAfterMove(Piece, toRow, toColumn, opponentPiece)
    ) {
...
const canPieceMove = {
    ...
    const moveTypes = {
      ...
      hideKing: () => {
        ...
        if (fromColumn === toColumn) {
          ...
          if (toRow === 4 && fromRow === 6 && !cells[5][fromColumn])
            return true;
        } else {
          return canPieceMove["Pawn"](
            fromRow,
  ...
  King: (fromRow, fromColumn, toRow, toColumn, moveType) => {
    const rowDifference = toRow - fromRow;
    const columnDifference = toColumn - fromColumn;
    if (moveType === "killPiece" || moveType === "hideKing")
      moveType = "makeCheck";
    const moveTypes = {
      makeCheck: () => {
        if (
        ...
        if (
          ...
          if (toRow === 7 && piece.row === 7) {
            ...
            if (columnDifference === -2) {
              if (
                piecesForCastlingNeverMoved.leftRook &&
            ...
            if (columnDifference === 2) {
              if (
                piecesForCastlingNeverMoved.rightRook &&
                piecesForCastlingNeverMoved.king
              ) {
