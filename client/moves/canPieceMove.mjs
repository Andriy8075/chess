...
const pawnMoveTypes = {
  makeCheckForOurKing: ({ fromRow, toRow, fromCol, toCol }) => {
    ...
    return (
      fromRow - toRow === -1 && (colDifference === 1 || colDifference === -1)
    );
  },
  ...
  passant: ({ fromRow, toRow, fromCol, toCol }) => {
    ...
    const isCheck = checkAfterMove({ piece: ourPiece, toRow, toCol });
  ...
  withoutKill: ({ fromRow, toRow, fromCol, toCol }) => {
    ...
    if (fromCol === toCol) {
      ...
      if (
        toRow === startPawnRow - 2 &&
        fromRow === startPawnRow &&
        !isPieceOnIntermediateCell
      )
        return true;
    }
    return false;
  ...
  bishop: ({ fromRow, fromCol, toRow, toCol }) => {
    ...
    if (rowDifference === colDifference) {
      if (rowDifference > 0) {
        ...
        // bishop goes in right and bottom
        let col = fromCol - 1;
        for (let row = fromRow - 1; row > toRow; row--, col--) {
          const pieceOnCell = cells[row][col];
          if (pieceOnCell) return false;
    ...
    if (rowDifference === -colDifference) {
      if (rowDifference > 0) {
        ...
        // bishop goes in left and bottom
        let col = fromCol + 1;
        for (let row = fromRow - 1; row > toRow; row--, col++) {
          const pieceOnCell = cells[row][col];
          if (pieceOnCell) return false;
