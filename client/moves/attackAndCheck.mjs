...
const attack = ({
  ...
  toCol,
  ignorePieces,
  moveType = "makeCheckForOurKing",
  firstPieceId,
}) => {
  let lastPieceId;
  if (firstPieceId) {
    lastPieceId =
      firstPieceId <= fistColorPieceMaxId ? fistColorPieceMaxId : countOfPieces;
  } else {
    [firstPieceId, lastPieceId] =
      color === "white"
        ? [1, fistColorPieceMaxId]
        : [fistColorPieceMaxId + 1, countOfPieces];
  }
...
const checkAfterMove = ({ piece, toRow, toCol, killPiece }) => {
  ...
  changeCell(previousRow, previousCol, undefined);
