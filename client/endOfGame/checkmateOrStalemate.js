...
const checkmateOrStalemate = () => {
    ...
    if (!attackingPiece) return stalemate(); else {
            ...
            if (row >= 0 && row < 8) {
                for (let column = vars.kingColumn - 1; column <= vars.kingColumn + 1; column++) {
                    if (column >= 0 && column < 8) {
                        ...
                        if (enemyPiece && enemyPiece.color === vars.oppositeColor && !checkAfterMove(king, row, column, enemyPiece)) {
                            return;
                        }
                    }
                }
        ...
        const secondPieceThatMakeCheck = attack(vars.color, vars.kingRow, vars.kingColumn, [attackingPiece], "makeCheck",);
        if (secondPieceThatMakeCheck) {
            return "checkmate";
        }
        let ignoringPieces = [king];
        const killAttackingPiece = (i) => {
            const savingPiece = attack(vars.oppositeColor, attackingPiece.row, attackingPiece.column, ignoringPieces, "killPiece", i,);
            if (savingPiece) {
        ...
        const hideKing = (cellRow, cellColumn, i) => {
            const savingPiece = attack(vars.oppositeColor, cellRow, cellColumn, ignoringPieces, "hideKing", i,);
            if (savingPiece) {
        ...
        if (!killAttackingPiece()) {
            ...
            const checkFromRook = () => {
                if (attackingPiece.column === vars.kingColumn) {
                    if (attackingPiece.row < vars.kingRow) {
                        for (let currentCellRow = vars.kingRow - 1; currentCellRow > attackingPiece.row; currentCellRow--) {
                    ...
                    if (attackingPiece.column < vars.kingColumn) {
                        for (let currentCellColumn = vars.kingColumn - 1; currentCellColumn > attackingPiece.column; currentCellColumn--) {
            ...
            const checkFromBishop = () => {
                if (attackingPiece.row < vars.kingRow) {
                    if (attackingPiece.column < vars.kingColumn) {
                        ...
                        for (let currentCellRow = vars.kingRow - 1; currentCellRow > attackingPiece.row; currentCellRow--, currentCellColumn--) {
            ...
            if (attackingPiece.type === "Queen") {
                if (attackingPiece.column === vars.kingColumn || attackingPiece.row === vars.kingRow) {
                    return checkFromRook();
