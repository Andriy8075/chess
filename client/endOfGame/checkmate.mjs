...
const hideKing = (toRow, toCol, firstPieceId) => {
  ...
  if (isCheckAfterMove) {
    ...
    return hideKing(toRow, toCol, savingPiece.id + 1);
  }
  return true;
};
...
const checkmate = () => {
  ...
  if (attackingPiece.type === "queen") {
    if (attackingPiece.col === kingCol || attackingPiece.row === kingRow) {
      return checkFromRook();
    }
    return checkFromBishop();
  }
