...
const moveExist = {
  ...
  bishop: (piece) => {
    if (piece.col > 0 && piece.row > 0) {
      if (canMoveTo(piece, piece.row - 1, piece.col - 1)) {
        return true;
      }
    }
  ...
  pawn: (piece) => {
    ...
    if (piece.col > 0) {
      ...
      if (pieceToKillId) {
        ...
        if (
          pieceToKill.color === gameState.oppositeColor &&
          !checkAfterMove({
            piece,
            toRow: piece.row - 1,
            toCol: piece.col - 1,
  ...
  knight: (piece) => {
    if (piece.col > 0) {
      if (piece.row > 1)
        if (canMoveTo(piece, piece.row - 2, piece.col - 1)) return true;
      if (piece.row < maxRowAndCol - 1)
        if (canMoveTo(piece, piece.row + 2, piece.col - 1)) return true;
      if (piece.col > 1) {
        if (piece.row > 0)
          if (canMoveTo(piece, piece.row - 1, piece.col - 2)) return true;
        if (piece.row < maxRowAndCol)
          if (canMoveTo(piece, piece.row + 1, piece.col - 2)) return true;
      }
    }
...
const stalemate = () => {
  for (const piece of pieces) {
    if (
      piece &&
      piece.color === gameState.color &&
      moveExist[piece.type](piece)
    )
      return false;
  }
  return true;
